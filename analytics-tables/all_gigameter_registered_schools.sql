...
SELECT
  master.country,
  master.iso3_code,
  master.school_id_giga,
  master.school_name,
  master.school_id_govt,
  -- registration info
  case when r.first_registration_attempt is not null then 'Yes' else 'No' end as registered_gigameter,
  case when data.first_measurement_date is not null then 'Yes' else 'No' end as sending_gigameter_data,
  data.first_measurement_date,
  ...
  -- devices 
  r.num_devices_registered,
  r.devices_still_logged_in,
  data.num_devices_measured,
  app.max_app_version as max_app_version_gigameter,
  -- provider info (isp and server)
