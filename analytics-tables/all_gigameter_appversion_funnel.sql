...
SELECT DISTINCT
    -- Funnel flags
    CASE WHEN d.initial_registration_date IS NULL THEN 'no' ELSE 'yes' END AS registered_gigameter,
    CASE WHEN i.first_measurement_date IS NULL THEN 'no' ELSE 'yes' END AS sent_gigameter_data,
    ...
    d.school_id_govt,
    d.device_id,
    d.user_id as browser_id,
    
    -- RT source identification
    CASE WHEN lower(d.initial_app_version) LIKE '%mlab%' THEN 'mlab' ELSE 'GigaMeter' END AS initial_rt_source,
    ...
    -- Important dates
    i.first_measurement_date,
    d.initial_registration_date,
    r.last_measurement_date,
    vu.version_update_date,
    
    -- Version upgrade information
    vu.app_version AS version_updated_to,
    
    -- Formatted version strings (for display)
    COALESCE(NULLIF(d.initial_app_version, '') || ' (initial)', 'null (initial)') AS initial_app_version,
    COALESCE(NULLIF(i.first_app_version, '') || ' (first)', 'null (first)') AS first_app_version_measured,
    COALESCE(NULLIF(vu.app_version, '') || ' (current)', 'null (current)') AS most_recent_app_version
